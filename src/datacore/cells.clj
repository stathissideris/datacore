(ns datacore.cells
  ...
  (:require [datacore.cells.macros :refer :all]))
...
(def ^:private ^:dynamic *tx* (sorted-set-by cmp-rank))
...
(defn- add-sync!  [c]   (conj *tx* c))
(defn- safe-nth   [c i] (try (nth c i) (catch Exception _)))
(defn- propagate! [c]   (if *tx* (doto c add-sync!) (doto c (-> to-array propagate*))))
...
(defn set-formula!
  ...
  ([this f]
   (destroy-cell! this true)
   (set-formula!* this f (to-array []) nil))
  ([this f sources]
   (destroy-cell! this true)
...
(defprotocol CellARef
  (notifyWatches [this o n]))

(deftype Cell [meta
               ^:unsynchronized-mutable state
               ^:unsynchronized-mutable rank
               ^:unsynchronized-mutable prev
               ^:unsynchronized-mutable sources
               ^:unsynchronized-mutable sinks
               ^:unsynchronized-mutable thunk
               ^:unsynchronized-mutable watches
               ^:unsynchronized-mutable update
               ^:unsynchronized-mutable constant
               ^:unsynchronized-mutable numwatches]
  ;; clojure.lang.IPrintWithWriter
  ;; (-pr-writer [this w _]
...
(defmethod print-dup Cell [o w]
  (.write w "#object [datacore.cells.Cell " (pr-str (.-state o)) "]"))
...
(defn cell
  ...
  ([x] (Cell. nil x (next-rank) x (to-array []) (to-array []) nil {} nil false 0))
  ([x & {:keys [meta]}] (Cell. meta x (next-rank) x (to-array []) (to-array []) nil {} nil false 0)))
