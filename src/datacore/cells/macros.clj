...
(let [to-list   #(into '() (reverse %))
  ...
  (defn cell* [x env]
    (let [[f args] (hoist x env)]
      (to-list `((datacore.cells/formula ~f) ~@args)))))
...
(defmacro formula-of
  ...
  `((datacore.cells/formula (fn [~@deps] ~@body)) ~@deps))
...
(defmacro formulet
  ...
  (let [binding-pairs (partition 2 bindings)]
    ...
    `((datacore.cells/formula (fn [~@(map first binding-pairs)] ~@body))
      ~@(map second binding-pairs))))

(defmacro ^:private cell-let-1 [[bindings c] & body]
  (let [syms  (bind-syms bindings)
        dcell `((datacore.cells/formula (fn [~bindings] [~@syms])) ~c)]
...
(defmacro cell-doseq
  ...
  (if (= 2 (count bindings))
    `(cell-doseq*
       ((datacore.cells/formula seq) ~(second bindings))
    ...
    (let [pairs   (partition 2 bindings)
      ...
      `(cell-doseq*
         ((datacore.cells/formula (fn [~@gens] (for [~@fors] [~@syms]))) ~@exprs)
